...
function Stock() {
    ...
    const current = stock.slice(indexOfFirst, indexOfLast);
    ...
    async function get(e) {
        ...
        await axios.get(`https://localhost:44398/stock/get/?id=${id}`)
            .then(res => {
    ...
    async function update(e) {
        ...
        if (checkInput()) {
            ...
            axios({
                method: 'PUT',
                url: 'https://localhost:44398/stock',
    ...
    async function del(e) {
        ...
        axios.defaults.withCredentials = true;
        setLoading(true);
        await axios.delete(`https://localhost:44398/stock/?id=${id}`)
    ...
    return (
        <div className='d-flex flex-column justify-content-center' style={{ width: "98%" }}>
                    ...
                    {current.map(data => {
                        return (
                            <tr key={counter++}>
                                ...
                                <td>{data.price.toFixed(2)} Azn</td>
            ...
            <Pagination postsPerPage={postsPerPage} totalPosts={stock.length} paginate={paginate} />
                    ...
                    <div className="modal-content">
                        ...
                        <form>
                            <div className="modal-body">
                                <div className="mb-3">
                                    <label htmlFor="name" className="col-form-label">Material adı:</label>
                                    <input type="text" onChange={e => setName(e.target.value)} value={name} className="form-control" id="name" />
                                </div>
                                <div className="mb-3">
                                    <label htmlFor="price" className="col-form-label">Qiymət:</label>
                                    <input type="number" min={1} onChange={e => setPrice(e.target.value)} value={price} className="form-control" id="price" />
                                </div>
                                <div className="mb-3">
                                    <label htmlFor="count" className="col-form-label">Material sayısı:</label>
